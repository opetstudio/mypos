...
import { Button, Form, Grid, Header, Image, Message, List, Container } from 'semantic-ui-react'
  ...
  renderFields () {
    ...
    for (var key in this.props.myProfile) {
      if (this.props.myProfile.hasOwnProperty(key)) {
        ...
        r.push(<List.Item key={key} >
          <List.Icon name='marker' />
          <List.Content>
            <List.Header as='a'>{key}</List.Header>
            <List.Description>
              {this.props.myProfile[key]}
            </List.Description>
          </List.Content>
        </List.Item>)
      }
