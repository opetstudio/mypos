...
export function * getConference (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ConferenceActions.conferenceRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(ConferenceActions.conferenceRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postConference (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(ConferenceActions.conferenceCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(ConferenceActions.conferenceCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(ConferenceActions.conferenceCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateConference (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ConferenceActions.conferenceUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(ConferenceActions.conferenceUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeConference (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ConferenceActions.conferenceRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getConferences (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_conference')
    yield put(ConferenceActions.conferenceRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
