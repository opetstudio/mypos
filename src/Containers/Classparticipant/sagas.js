...
export const projectorColumns = ['_id,', 'class_id', 'participant_id', 'status', 'is_evaluated']
...
export function * getClassparticipant (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ClassparticipantActions.classparticipantRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(ClassparticipantActions.classparticipantRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postClassparticipant (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(ClassparticipantActions.classparticipantCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(ClassparticipantActions.classparticipantCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(ClassparticipantActions.classparticipantCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateClassparticipant (api, action) {
  ...
  const s = yield select(session)
  const response = yield call(api.updateClassparticipant, data, id, {session: s})
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  // success?
  if (response.ok) {
    ...
    yield put(ClassparticipantActions.classparticipantUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(ClassparticipantActions.classparticipantUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
export function * updateClassparticipantBatch (api, action) {
  ...
  // const params = yield select(theData)
  const s = yield select(session)
  const response = yield call(api.updateClassparticipantBatch, data, {session: s})
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  // success?
  if (response.ok) {
    // const { contentDetail, status, msg } = response.data
...
export function * removeClassparticipant (api, action) {
  ...
  // make the call to the api
  const s = yield select(session)
  const response = yield call(api.removeClassparticipant, data, id, {session: s})
  ...
  if (response.ok) {
    ...
    yield put(ClassparticipantActions.classparticipantRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getClassparticipants (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_class_participants', projectorColumns)
    yield put(ClassparticipantActions.classparticipantRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
