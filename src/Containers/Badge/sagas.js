...
export function * getBadge (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(BadgeActions.badgeRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(BadgeActions.badgeRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postBadge (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(BadgeActions.badgeCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(BadgeActions.badgeCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
...
export function * updateBadge (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(BadgeActions.badgeUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(BadgeActions.badgeUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeBadge (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(BadgeActions.badgeRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getBadges (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_badges')
    yield put(BadgeActions.badgeRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
