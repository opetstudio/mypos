...
export function * getFile (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FileActions.fileRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(FileActions.fileRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postFile (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201 && response.data._id !== undefined) {
      ...
      yield put(FileActions.fileCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(FileActions.fileCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    let validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    // if (path(['originalError', 'response', 'status'], response) === 500) return yield put(LoginActions.loginRemoveSuccess({}))
    yield put(FileActions.fileCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateFile (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FileActions.fileUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    // let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    let validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    // if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(FileActions.fileUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeFile (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FileActions.fileRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getFiles (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_files')
    yield put(FileActions.fileRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
