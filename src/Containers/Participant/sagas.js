...
export function * getParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ParticipantActions.participantRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(ParticipantActions.participantRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(ParticipantActions.participantCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(ParticipantActions.participantCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(ParticipantActions.participantCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ParticipantActions.participantUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(ParticipantActions.participantUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ParticipantActions.participantRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getParticipants (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_participant')
    yield put(ParticipantActions.participantRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
...
export function * participantFetchInspect (api, action) {
  ...
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  if (response.ok) {
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_inspect')
    yield put(ParticipantActions.participantFetchInspectDone({inspectById: byId, inspectAllIds: allIds, maxModifiedon}))
  }
}
