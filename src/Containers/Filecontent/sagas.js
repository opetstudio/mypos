...
export function * getFilecontent (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FilecontentActions.filecontentRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(FilecontentActions.filecontentRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postFilecontent (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(FilecontentActions.filecontentCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(FilecontentActions.filecontentCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(FilecontentActions.filecontentCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateFilecontent (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FilecontentActions.filecontentUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(FilecontentActions.filecontentUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeFilecontent (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(FilecontentActions.filecontentRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getFilecontents (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_filecontent')
    yield put(FilecontentActions.filecontentRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
