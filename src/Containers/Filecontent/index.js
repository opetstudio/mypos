...
class TheComponent extends Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (!_.isEqual(prevProps.fileDetail, this.props.fileDetail) && !_.isEmpty(this.props.fileDetail)) this.setState({fileDetail: Immutable.asMutable(this.props.fileDetail, {deep: true})})
  ...
  render () {
    ...
    return <LayoutTableData
      ...
      parentDetail={[{i: 0, title: 'File Detail', data: this.state.fileDetail}]}
      subMenu={[
        {label: 'Extract Data File', to: '', iconName: '', isLink: true, onClick: () => {}},
        {label: 'Submit Selected Rows', to: '', iconName: '', isLink: true, onClick: () => {}},
        {label: 'Submit All Rows', to: '', iconName: '', isLink: true, onClick: () => {}}
      ]}
    />
  }
}
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(FilecontentSelectors.getMaxModifiedon(state.filecontent) || 0),
    allDataArr,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAll: (query) => dispatch(FilecontentActions.filecontentRequestAll(query)),
    deleteRow: (query) => dispatch(FilecontentActions.filecontentDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(FilecontentActions.filecontentUpdate(data, id)),
    removeOne: (data, id) => dispatch(FilecontentActions.filecontentRemove(data, id)),
    updateBatch: (data) => dispatch(FilecontentActions.filecontentUpdateBatch(data))
  }
}
