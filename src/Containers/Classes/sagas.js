...
export function * doEvaluatedParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    if (status) {
      ...
      yield put(ClassparticipantActions.classparticipantRequestSuccess({requestMessage: '', byId, allIds}))

      const participantState = yield select(getParticipantState)
      const allDataInspectArr = ParticipantSelectors.getAllDataInspectArr(participantState)
      const allDataInspectArrImmutable = Immutable.asMutable(allDataInspectArr, {deep: true})
      console.log('allDataInspectArrImmutable', allDataInspectArrImmutable)
      let inspectDetailArr = _.filter(allDataInspectArrImmutable, {participant_id: theData.participant_id, class_id: theData.class_id}) || [{}]
      ...
      if (inspectDetail) {
        ...
        yield put(ParticipantActions.participantEvaluatedInspect({inspectById: inspectById}))
      }
    }
  }
  yield put(ClassesActions.classesEvaluatedParticipantDone(response.data))
...
export function * doDeleteParticipant (api, action) {
  ...
  if (response.ok) {
    ...
    if (status) {
      ...
      const allClassParticipant = ClassparticipantSelectors.getAllDataArr(clsPartSt)
      const allClassParticipantMutable = Immutable.asMutable(allClassParticipant, {deep: true})

      // console.log('allClassParticipant==>', allClassParticipantMutable)
      // console.log('classId==>', classId)
      // console.log('participantId==>', participantId)
      const classParticipantId = ((_.filter(allClassParticipantMutable, {class_id: classId, participant_id: participantId}) || []).map(o => o._id) || [0])[0]
      // console.log('classParticipantId==>', classParticipantId)
      yield put(ClassparticipantActions.classparticipantRemoveSuccess({id: classParticipantId}))
      const currentClassParticipants = Immutable.asMutable(classFormValue['class_participants'] || [], {deep: true})
      console.log('classFormValue==>', classFormValue)
      console.log('currentClassParticipants==>', currentClassParticipants)
      currentClassParticipants.splice(currentClassParticipants.indexOf(participantId), 1)
      yield put(ClassesActions.classesSetFormValue({class_participants: currentClassParticipants}))
    }
...
export function * postClasses (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(ClassesActions.classesCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(ClassesActions.classesCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(ClassesActions.classesCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateClasses (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ClassesActions.classesUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    else validationMessages = path(['data', 'detail'], response)
    yield yield put(ClassesActions.classesUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeClasses (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(ClassesActions.classesRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getClassess (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_classes')
    yield put(ClassesActions.classesRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
