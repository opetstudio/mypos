...
export function * getUser (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(UserActions.userRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(UserActions.userRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * getUserProfile (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(UserActions.userRequestProfileSuccess({requestMessage: 'success fetch data', detail}))
  } else {
    yield put(UserActions.userRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postUser (api, action) {
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(UserActions.userCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(UserActions.userCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(UserActions.userCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateUser (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(UserActions.userUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    if (response.status === 403) validationMessages = path(['originalError', 'response', 'statusText'], response)
    yield yield put(UserActions.userUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * removeUser (api, action) {
  ...
  if (response.ok) {
    ...
    yield put(UserActions.userRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getUsers (api, action) {
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_users')
    yield put(UserActions.userRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon}))
